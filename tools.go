...
// with the receiver with the reciever *Tools
type Tools struct {
	MaxFileSize        int
...
// UploadedFile is a struct to save information about the uplaoded file
...
// Slugify is a very simple mean of creating a slug
// from a string
...
// and avoid the browser from displaying and forces download
...
// WriteJSON takes a response status cdoe and a artbitrary data and write json to the client
...
// ErrorJSON takes an error and optionally a statuscode and generates and sends a JSON error message
...
// PushJSONToRemote posts arbitrary data to some URL as JSON and returns
// the response, statucode and error if any. The final parameter, client,
// is optional. If there is none specified we use the standard http.Client
